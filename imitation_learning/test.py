import sys

sys.path.append(".")
from datetime import datetime
import numpy as np
import gymnasium as gym
import os
import json

from agent.bc_agent import BCAgent
from utils import *


def states_with_history (dataset, history_length=1):
    size, h, w = dataset.shape
    dataset_ext = np.zeros(shape=(size, history_length, h, w))

    image_hist = []
    image_hist.extend([dataset[0]] * history_length)
    dataset_ext[0] = np.array(image_hist)
    for i in range(1, len(dataset)):
        image_hist.append(dataset[i])
        image_hist.pop(0)
        dataset_ext[i] = np.array(image_hist)
    return dataset_ext

def run_episode(env, agent, rendering=True, max_timesteps=1000, history_length=1):

    episode_reward = 0
    step = 0

    state, _ = env.reset()


    if rendering:
        # Gymnasium CarRacing uses pygame backend; render once after reset
        # to initialize/display a valid first frame.
        env.render()


    #For States history
    image_hist = []
    image_hist.extend([state] * history_length)
    state = np.array(image_hist)

    while True:

        # TODO: preprocess the state in the same way than in your preprocessing in train_agent.py
        state = np.expand_dims(rgb2gray(state)/ 255.0, axis=0)

        # TODO: get the action from your agent! You need to transform the discretized actions to continuous
        # actions.
        # hints:
        #       - the action array fed into env.step() needs to have a shape like np.array([0.0, 0.0, 0.0])
        #       - just in case your agent misses the first turn because it is too fast: you are allowed to clip the acceleration in test_agent.py
        #       - you can use the softmax output to calculate the amount of lateral acceleration
        a = id_to_action(torch.argmax(agent.predict(state)))
        next_state, r, terminated, truncated, _ = env.step(a)

        #For states history
        image_hist.append(next_state)
        image_hist.pop(0)
        next_state = np.array(image_hist)


        episode_reward += r
        state = next_state
        step += 1

        if rendering:
            env.render()

        if terminated or truncated or step > max_timesteps:
            break

    return episode_reward


if __name__ == "__main__":

    # important: don't set rendering to False for evaluation (you may get corrupted state images from gym)
    rendering = True
    n_test_episodes = 15  # number of episodes to test
    history_length = 1

    # TODO: load agent
    agent = BCAgent(lr=1e-4, history_length=history_length)
    model_dir = "./models/IL/"
    for model in {"bc_agent_carracing.pt", "bc_agent_carracing_besteval.pt"}:
        print (model)
        agent.load(os.path.join(model_dir, model))
        #Turn on Eval mode
        agent.net.eval()

        # env = gym.make("CarRacing-v3", render_mode="rgb_array")
        env = gym.make("CarRacing-v3", render_mode="human")

        episode_rewards = []
        for i in range(n_test_episodes):
            print ("Test episode #",i)
            episode_reward = run_episode(env, agent, rendering=rendering, max_timesteps=1000, history_length=history_length)
            episode_rewards.append(episode_reward)
            print(episode_reward)

        
        # save results in a dictionary and write them into a .json file
        results = dict()
        results["episode_rewards"] = episode_rewards
        results["mean"] = np.array(episode_rewards).mean()
        results["std"] = np.array(episode_rewards).std()

        print (np.array(episode_rewards).mean(), np.array(episode_rewards).std() )

        fname = f"results/results_bc_agent-{datetime.now().strftime('%Y%m%d-%H%M%S')}.json"
        with open(fname, "w") as f:
            json.dump(results, f)

    env.close()
    print("... finished")
